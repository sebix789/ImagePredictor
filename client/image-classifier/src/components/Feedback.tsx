...
const Feedback: React.FC<FeedbackProps> = ({ imageName, prediction }) => {
  ...
  useEffect(() => {
      const fetchLabels = async () => {
          try {
              ...
              const combinedLabels = original.map((label: string, index: string | number) => ({
                original: label,
                formatted: formatted[index]
              }))

              combinedLabels.sort((a: any, b: any) => a.formatted.localeCompare(b.formatted))

              setOriginalLabels(combinedLabels.map((item: { original: string; formatted: string }) => item.original));
              setFormattedLabels(combinedLabels.map((item: { original: string; formatted: string }) => item.formatted));
          } catch (error) {
  ...
  return (
      <Box sx={{ marginTop: "20px", textAlign: "center" }}>
        ...
        {isCorrect === false && (
          <Box sx={{ marginTop: "20px", display: "flex", justifyContent: "center", marginBottom: "20px" }}>
